...
public class DeviceProfile {
    ...
    private void hideWorkspaceLabelsIfNotEnoughSpace() {
		...
		if("1".equals(SystemProperties.get("ro.config.launcher_style12"))){
			iconTextHeight = Utilities.calculateTextHeight(iconTextSizePx)* 2;
		}
    ...
    public void autoResizeAllAppsCells() {
		...
		if("1".equals(SystemProperties.get("ro.config.launcher_style12"))){
			allAppsCellHeightPx = allAppsIconSizePx + allAppsIconDrawablePaddingPx
                + textHeight * 2 + (topBottomPadding * 2);
		}
    ...
    public void updateIconSize(float scale, Resources res) {
		...
		if("1".equals(SystemProperties.get("ro.config.launcher_style12"))){
		      updateIconSizeExt(mMetrics, isMultiWindowMode);
              cellWidthPx = iconSizePx + iconDrawablePaddingPx;
		}
	...
	public void updateIconSizeExt(DisplayMetrics dm, boolean isMultiWindowMode) {
        ...
        int oldTextRectHeight = cellHeightPx - iconSizePx;
        ...
        if (iconDrawablePaddingPx > cellYPadding
            ...
            if (cellYPadding > 0) {
                ...
                if (adjustPadding > 0) {
                    ...
                    cellHeightPx = cellHeightPxWithoutPadding;
                    iconDrawablePaddingPx = 0;
                    int newTextHeight = cellHeightPx - iconSizePx;
                    float iconTextScale = (float) newTextHeight / oldTextRectHeight;
                    iconTextSizePx = (int) (iconTextSizePx * iconTextScale);

                    verifyIconTextSize(dm);
                }
            }
        }
    }
	
	private void verifyIconTextSize(DisplayMetrics dm) {
        int minIconTextSizePx = Utilities.pxFromSp(8.0f, dm);
        if (iconTextSizePx < minIconTextSizePx) {
            int oldIconHeight = calculateIconHeight(iconTextSizePx);
            int newIconHeight = calculateIconHeight(minIconTextSizePx);
            float iconScale = (float) newIconHeight / oldIconHeight;
            iconSizePx = (int) (iconSizePx * iconScale);
            iconTextSizePx = minIconTextSizePx;
        }
    }
	
	private int calculateIconHeight(int iconTextSizePxPara) {
        return cellHeightPx - iconDrawablePaddingPx
                - Utilities.calculateTextHeight(iconTextSizePxPara) * 2;
    }
    ...
    private void updateFolderCellSize(float scale, Resources res) {
		...
		if("1".equals(SystemProperties.get("ro.config.launcher_style12"))){
				updateFolderCellSizeExt();
		}		
				
    }
	
	 public void updateFolderCellSizeExt() {
       
            folderChildTextSizePx = Math.min(iconTextSizePx, folderChildTextSizePx);
            int textHeight = Utilities.calculateTextHeight(folderChildTextSizePx) * 2;
            int minCellPaddingY = mContext.getResources()
                    .getDimensionPixelSize(R.dimen.min_folder_cell_y_padding);

            folderCellHeightPx = folderChildIconSizePx + minCellPaddingY + textHeight;
            folderChildDrawablePaddingPx = Math.max(0,
                    (folderCellHeightPx - folderChildIconSizePx - textHeight) / 3);
